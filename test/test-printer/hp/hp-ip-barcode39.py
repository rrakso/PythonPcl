...
def print_barcode_doc(printer_socket):
    ...
    d.writeln("This a demonstration document")
    d.writeln("including BarCode39 generation")
    d.writeln()
    d.writeln("You can read the barcode with a product like Barcode")
    d.writeln(" Reader/Scanner Module - CCD Camera - USB Interface")
    d.writeln("Such module is available at http://shop.mchobby.be")
    d.writeln("Those module are usually configured for Qwerty Keyboards")
    d.writeln(" where numbers are accessible on the first row")
    d.writeln("When using French keyboards, you have to press the")
    d.writeln(" Shift key (or Shift Lock) to activate the numeric")
    d.writeln(" value on the first row")
    d.writeln("Surprisingly, on Linux Mint, you have to press down the")
    d.writeln(" RIGHT SHIFT KEY to activate numbers on the first row!")
    d.writeln(" The best option is certainly to reconfigure your scanner")
    d.writeln()
    d.writeln("Now, enjoy the Barcode 39 reading with your scanner :-) ")
    d.writeln()

    bc = Barcode39(d)  # Owner must be the document
    bc.write("MCHP00189")  # Write the bar code into the document
    d.writeln()
    d.writeln("    " + bc.barcode_message("MCHP00189"))
    d.writeln()

    d.pitch(15)  # 15 characters per inch
    bc.write("GGN00030365")  # Write the barcode in the document
    d.writeln()
    d.writeln("    " + bc.barcode_message("GGN00030365"))
