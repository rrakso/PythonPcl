...
class PclDataType(object):
    ...
    # Content is supposed to be BINARY content (contained into bytes)
    BINARY = 3

    items = {PCL: 'PCL command stream',
             TEXT: 'Text stream', BINARY: 'Bytes stream'}
...
class PclDocument(list):
    ...
    """
    PRINTER_CRLF = chr(
        13)+chr(10)  # Go to begin of next line, should be override in descendant class
    ...
    def __init__(self, target_encoding='cp850', printer_adapter=None, title=''):
        ...
        printer_adapter : (PrinterAdapter) - object derived from
                                          ...
                                          been fixed later."""
        assert isinstance(
            target_encoding, str), 'target_encoding must be a string. eg: cp850 or utf8'
