...
class PclDocument(list):
    ...
    PRINTER_CRLF = chr(13) + chr(
        10
    )  # Go to begin of next line, should be override in descendant class
    ...
    def __init__(self, target_encoding="cp850", printer_adapter=None, title=""):
        ...
        assert isinstance(
            target_encoding, str
        ), "target_encoding must be a string. eg: cp850 or utf8"
    ...
    def append(self, item):
        assert isinstance(
            item, tuple
        ), "PclDocument can only append tuple (pcl_type!=binary, unicode_data) or (pcl_type==binary, bytes)"
        ...
        assert ((item[0] != PCL_DATA_TYPE.BINARY) and isinstance(item[1], str)) or (
            (item[0] == PCL_DATA_TYPE.BINARY) and isinstance(item[1], bytes)
        ), "tuple[1] must be an str string or binary!"
